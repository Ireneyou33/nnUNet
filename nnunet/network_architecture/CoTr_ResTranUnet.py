...
import numpy as np
from nnunet.network_architecture import CoTr_CNNBackbone
...
class U_ResTran3D(nn.Module):
    def __init__(self, norm_cfg='BN', activation_cfg='ReLU', img_size=None, num_classes=None, weight_std=False):
        ...
        self.backbone = CoTr_CNNBackbone.Backbone(in_channels=4, depth=9, norm_cfg=norm_cfg, activation_cfg=activation_cfg, weight_std=weight_std)
    ...
    def forward(self, inputs):
        ...
        # # skip2 = x_convs[-2]
        # Multi-scale 
        # print("shape::::::::::::::::::::::::::::::",len(x_trans),len(x_trans[0]),len(x_trans[0][0]), len(x_convs[3]),len(x_convs[2]),len(x_convs[1]),len(x_convs[0][0]))
        #import pdb
        #pdb.set_trace()
        x = self.transposeconv_stage2(x_trans[:, 8192::].transpose(-1, -2).view(x_convs[-1].shape)) # x_trans length: 12*24*24+6*12*12=7776
        skip2 = x_trans[:, 0:8192].transpose(-1, -2).view(x_convs[-2].shape)
