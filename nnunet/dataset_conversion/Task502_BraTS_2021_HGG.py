...
if __name__ == "__main__":
    ...
    for p in subdirs(cur, join=False):
        ...
        t1_out = join(target_imagesTr, patient_name + "_0000.nii.gz")
        if not isfile(t1_out):
            shutil.copy(t1, t1_out)
            print(t1_out)
        t1c_out = join(target_imagesTr, patient_name + "_0001.nii.gz")
        if not isfile(t1c_out):
            shutil.copy(t1c, t1c_out)
            print(t1c_out)
        t2_out = join(target_imagesTr, patient_name + "_0002.nii.gz")
        if not isfile(t2_out):
            shutil.copy(t2, t2_out)
            print(t2_out)
        flair_out = join(target_imagesTr, patient_name + "_0003.nii.gz")
        if not isfile(flair_out):
            shutil.copy(flair, flair_out)
            print(flair_out)

        seg_out = join(target_labelsTr, patient_name + ".nii.gz")
        if not isfile(seg_out):
            copy_BraTS_segmentation_and_convert_labels(seg, seg_out)
            print(seg_out)
