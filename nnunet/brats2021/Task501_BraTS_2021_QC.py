...
def get_qc_info(seg_file, t1_file, t1ce_file, t2_file, flair_file, sub_id, grading):
    """
    获取QC需要的信息
    """
    ...
    subid = sub_id
    modailty = 4
    label = np.unique(img_npy)
    demensions = img_npy.shape
    voxel_size = img.GetSpacing()
    dicom_filter = sitk.DICOMOrientImageFilter()
    orientation = dicom_filter.GetOrientationFromDirectionCosines(img.GetDirection())
    label1_size = np.sum(img_npy == 1)
    label2_size = np.sum(img_npy == 2)
    label4_size = np.sum(img_npy == 4)
    HGG_LGG = grading
    z_range = np.where((img_npy != 0))[0].min(), np.where((img_npy != 0))[0].max()

    img = sitk.ReadImage(t1_file)
    img_npy = sitk.GetArrayFromImage(img)
    t1_range = img_npy.min(), img_npy.max()

    img = sitk.ReadImage(t1ce_file)
    img_npy = sitk.GetArrayFromImage(img)
    t1ce_range = img_npy.min(), img_npy.max()

    img = sitk.ReadImage(t2_file)
    img_npy = sitk.GetArrayFromImage(img)
    t2_range = img_npy.min(), img_npy.max()

    img = sitk.ReadImage(flair_file)
    img_npy = sitk.GetArrayFromImage(img)
    flair_range = img_npy.min(), img_npy.max()
    ...
    data = {
        ...
        'label2_size': label2_size,
        'label4_size': label4_size,
        'HGG_LGG': HGG_LGG,
        'z_range': z_range,
        't1_range': t1_range,
        't1ce_range': t1ce_range,
        't2_range': t2_range,
        'flair_range': flair_range,
    }
    print(data)
    return data
...
if __name__ == "__main__":
    ...
    for p in subdirs(cur, join=False):
        ...
        if df_grad is not None:
            ...
            grad = ' '
        qc_info = get_qc_info(seg, t1, t1c, t2, flair, p, grading=grad)
        df = df.append(qc_info, ignore_index=True)
    df.to_csv('BraTS2021_QC_2021-07-13.csv', index=False)
