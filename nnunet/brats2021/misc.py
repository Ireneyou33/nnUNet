...
def plot_voxel_enhance(arr, arr_mask=None, out_file=None, plot_type=None):  # arr: zxy
    ...
    if arr_mask is None or plot_type == 'origin':
        ...
        if plot_type == 'contourd':  # 添加掩码轮廓图
            ret, binary = cv2.threshold(res_mask_img, 1, 255, cv2.THRESH_BINARY)
            ...
            cv2.fillPoly(layer, contours, (0, 0, 255))
            dst = cv2.addWeighted(res_img, 1, layer, 0.3, 0)
            cv2.imwrite(out_file, dst)
        else:
            raise RuntimeError('plot_type is error')
...
def MAX_3D_REGION4(data_matrix):
    label_temp = label(data_matrix)
    props = []
    for prop in regionprops(label_temp):
        props.append(prop)
    props = sorted(props, key=lambda x: x['area'], reverse=True)
    labels = [i['label'] for i in props]
    label_temp[label_temp != labels[0]] = 0
    label_temp[label_temp == labels[0]] = 1
    return label_temp.astype(np.uint8)
